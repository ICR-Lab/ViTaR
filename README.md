...
To publish it in the `icr-lab` repository, copy the contents of this folder—including `index.html` and `assets/`—to that repository’s root. Then select **Settings → Pages → Deploy from a branch**, choose the deployment branch, and select `/(root)`.
